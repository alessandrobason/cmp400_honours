...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    tex[id] = min(sdf_sphere(pos, shape_pos, data.x), MAX_STEP);
#elif defined(SHAPE_BOX)
    tex[id] = min(sdf_box(pos, shape_pos, data.xyz), MAX_STEP);
#elif defined(SHAPE_CYLINDER)
    tex[id] = min(sdf_cylinder(pos, shape_pos, data.x, data.y), MAX_STEP);
#else
    tex[id] = MAX_STEP;
#endif
}
