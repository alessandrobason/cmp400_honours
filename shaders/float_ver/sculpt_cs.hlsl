...
inline void setVolumeTexture(uint3 id, float new_value, float3 pos) {
    ...
    setVTSkipRead(id, old_value, min(new_value, 16), pos);
}
...
inline void writeApproximateDistance(float3 pos, float dist, uint3 id) {
...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    if (dist > 0) {
        ...
        //     bool changed;
        //     op_union(sampleWorld(id), min(dist, 16), id, changed);
        // }
        return;
    }
    ...
    if (dist_from_tex > 0.) {
        bool changed;
        op_union(sampleWorld(id), dist_from_tex, id, changed);
        return;
    }
